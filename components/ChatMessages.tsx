...
export default function ChatMessages({ messages, user }: { messages: any[]; user: any }) {
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto p-4 sm:p-6 relative z-10">
      <div className="max-w-4xl mx-auto w-full space-y-4">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center py-20">
            <div className="w-20 h-20 bg-gradient-to-br from-pink-500/20 to-violet-500/20 rounded-2xl flex items-center justify-center mb-6 backdrop-blur-sm border border-white/10">
            ...
            <p className="text-white/40 max-w-sm">Start the conversation! Your messages will appear here.</p>
          </div>
        ) : (
          <>
            {messages.map((msg: any, idx: number) => (
              <div 
                key={idx} 
                className={`flex gap-3 ${msg.isMine ? "flex-row-reverse" : ""} animate-fade-in-up`}
              ...
              >
                ...
                {/* Message Content */}
                <div className={`flex-1 flex flex-col ${msg.isMine ? "items-end" : ""} max-w-[75%]`}>
                  ...
                  <div className={`group relative px-4 py-3 rounded-2xl transition-all duration-200 hover:scale-[1.02] ${
                    ...
                    {/* Decorative tail */}
                    <div className={`absolute bottom-3 w-3 h-3 rotate-45 ${
                      msg.isMine 
